...
class AppStore extends Component {
  ...
  onClickTabBtn = uniqueId => {
    console.log(uniqueId)
    this.setState({activeTabId: uniqueId})
  }

  getfilteredActiveTabId = () => {
    const {activeTabId} = this.state
    const filterobjects = appsList.filter(
  ...
  render() {
    ...
    const filteredActiveTabId = this.getfilteredActiveTabId()
    const filterSearchInput = filteredActiveTabId.filter(eachInputItem => {
      const eachItemInLowerCase = eachInputItem.appName.toLowerCase()
      return eachItemInLowerCase.includes(searchInput.toLowerCase())
    })

    return (
      <div className="bg-container">
        <h1 className="headEl">App Store</h1>
        ...
        <ul className="appItemContainer">
          {filterSearchInput.map(eachAppItem => (
            <AppItem appListItems={eachAppItem} key={eachAppItem.appId} />
          ))}
        </ul>
      </div>
    )
